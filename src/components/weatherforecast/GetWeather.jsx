...
function GetWeather(props) {
  ...
  function getData() {
    ...
    strong.current.innerText = vybrany_option.innerText

    const latitude = vybrany_option.dataset.latitude
    const longitude = vybrany_option.dataset.longitude

    const url = "https://api.open-meteo.com/v1/forecast?latitude="+latitude+"&longitude="+longitude+"&daily=weather_code,temperature_2m_max,wind_speed_10m_max,rain_sum,snowfall_sum&timezone=Europe%2FBerlin&forecast_days=3"
    ...
    axios.get(url).then(response => {
      ...
      for (let i = 0; i < days.time.length; i++) {
        let d = new Date(days.time[i])
        d = d.toLocaleString("sk-SK", {dateStyle: "short"})
  ...
  return (
    <>
      <p>{phrase["Find out the weather"]}: <strong ref={strong}>Košice</strong>
      </p>
      <select ref={select}>
      ...
      <button type="submit" onClick={getData} className="btn">{phrase["Find out"]}</button>
    </>
  )
}
